...
function DrawerOverlay({
  ...
  return (
    <DrawerPrimitive.Overlay
      ...
      className={cn(
        "data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 bg-primary/50 fixed inset-0 z-50 backdrop-blur-sm",
        className
      )}
      {...props}
    />
  ...
  return (
    <DrawerPortal data-slot="drawer-portal">
      ...
      <DrawerPrimitive.Content
        ...
        {...props}
      >
        <div className="bg-secondary mx-auto my-2 hidden h-2 w-24 shrink-0 rounded-full group-data-[vaul-drawer-direction=bottom]/drawer-content:block" />
        {children}
      </DrawerPrimitive.Content>
    </DrawerPortal>
  )
...
function DrawerHeader({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn(
        "flex w-full flex-col gap-1 rounded-t-3xl border-b px-4 py-3",
        className
      )}
      {...props}
    />
...
function DrawerBody({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn("w-full px-4 py-4 overflow-y-auto", className)}
      {...props}
    />
  )
}
