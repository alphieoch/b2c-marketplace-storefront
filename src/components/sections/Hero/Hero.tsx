  ...
  return (
    <section className="w-full flex container mt-5 flex-col lg:flex-row text-primary">
      ...
      <div className="w-full lg:order-2">
        <div className="w-full px-6 flex items-end h-[calc(100%-144px)]">
          <div>
          ...
          <div className="h-[72px] lg:h-[144px] flex font-bold uppercase">
            {buttons.map(({ label, path }) => (
              <Link
                key={path}
                href={path}
                className="group flex rounded-sm h-full w-1/2 bg-content hover:bg-action hover:text-tertiary transition-all duration-300 p-6 justify-between items-end"
                aria-label={label}
                title={label}
              >
                <span>
